...
class CardSpawner:
    ...
    async def increment_message_count(
        self, session: AsyncSession, guild_id: int, channel_id: int
    ):
        ...
        if server_config.message_threshold is None:
            server_config.message_threshold = random.randint(
                config.SPAWN_MESSAGE_MIN, config.SPAWN_MESSAGE_MAX
            )
    ...
    async def spawn_card(
        ...
        guild_id: int,
        channel_id: int,
        bypass_active_check: bool = False,
        ...
        try:
            ...
            embed.add_field(
                name="Rating", value=f"{card.overall_rating} OVR", inline=True
            )

            embed.set_footer(
                text=f"You have {config.CATCH_TIMEOUT_SECONDS} seconds to catch this card!"
            )
            ...
            expires_at = discord.utils.utcnow() + timedelta(
                seconds=config.CATCH_TIMEOUT_SECONDS
            )
            ...
            # Schedule expiration
            asyncio.create_task(
                self._handle_expiration(message, spawned_card.id, channel)
            )
    ...
    async def _handle_expiration(
        self, message: discord.Message, spawned_card_id: int, channel
    ):
    ...
    async def attempt_catch(
        ...
        result = await session.execute(
            select(SpawnedCard).where(SpawnedCard.id == spawn_data["spawned_card_id"])
        )
        ...
        if active_match:
            return (
                False,
                f"⚠️ You cannot catch cards while in an active match! Please complete your match in <#{active_match.channel_id}> first.",
            )
        ...
        collection_entry = Collection(user_id=user_id, card_id=card.id)
        ...
        return (
            True,
            f"Congratulations! You caught **{card.name}** ({card.overall_rating} OVR {card.position})!",
        )
...
class CatchCardModal(discord.ui.Modal, title="Catch the Card!"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if success:
            ...
            embed.add_field(
                name="Rating", value=f"{self.card.overall_rating} OVR", inline=True
            )
