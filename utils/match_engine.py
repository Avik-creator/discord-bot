...
class MatchEngine:
    ...
    @staticmethod
    def calculate_player_effective_stat(
        card: Card, position: str, formation: str, team_data: Dict, is_attacking: bool
    ) -> int:
        """
        ...
        attack_bonus, defense_bonus = FormationManager.apply_formation_bonuses(
            card, position, formation
        )
        if is_attacking:
            stat = attack_bonus
        else:
        ...
        chemistry_bonus = MatchEngine._calculate_player_chemistry(
            position, card, team_data
        )
...
class MatchState:
    ...
    def __init__(
        ...
        player1_team: Dict,
        player2_team: Dict,
        player1_formation: str,
        player2_formation: str,
    ):
        ...
        self.last_player1_position = (
            None  # Store P1's selection for the round (sets are unordered!)
        )
    ...
    def select_card(self, player_id: int, position: str) -> bool:
        ...
        if position not in available:
            logger.error(
                f"select_card: Position {position} not in available cards for player {player_id}"
            )
            logger.error(f"Available positions: {list(available.keys())}")
            if player_id == self.player1_id:
                logger.error(
                    f"Player 1 used positions: {self.player1_used_cards}, used card IDs: {self.player1_used_card_ids}"
                )
            else:
                logger.error(
                    f"Player 2 used positions: {self.player2_used_cards}, used card IDs: {self.player2_used_card_ids}"
                )
            return False
        ...
        if player_id == self.player1_id:
            ...
            logger.info(
                f"[BEFORE] Player 1 select_card: position={position}, card={card.name} (ID:{card.id})"
            )
            logger.info(f"[BEFORE] Player 1 used_positions: {self.player1_used_cards}")
            logger.info(
                f"[BEFORE] Player 1 used_card_ids: {self.player1_used_card_ids}"
            )

            # Double-check it's not already used
            if (
                position in self.player1_used_cards
                or card.id in self.player1_used_card_ids
            ):
                logger.error(
                    f"select_card: Card {card.name} (ID: {card.id}) at position {position} is already used for player 1!"
                )
                return False
            ...
            logger.info(
                f"select_card: Marked card {card.name} (ID: {card.id}) at position {position} as used for player 1"
            )
        else:
            ...
            # PARANOID LOGGING - Log state BEFORE modifying
            logger.info(
                f"[BEFORE] Player 2 select_card: position={position}, card={card.name} (ID:{card.id})"
            )
            logger.info(f"[BEFORE] Player 2 used_positions: {self.player2_used_cards}")
            logger.info(
                f"[BEFORE] Player 2 used_card_ids: {self.player2_used_card_ids}"
            )

            # Double-check it's not already used
            if (
                position in self.player2_used_cards
                or card.id in self.player2_used_card_ids
            ):
                logger.error(
                    f"select_card: Card {card.name} (ID: {card.id}) at position {position} is already used for player 2!"
                )
                return False
            ...
            logger.info(
                f"select_card: Marked card {card.name} (ID: {card.id}) at position {position} as used for player 2"
            )
    ...
    def play_round(self, player1_position: str, player2_position: str) -> Dict:
        ...
        logger.info(
            f"Round {self.current_round}: Player 1 using {player1_card.name} (ID: {player1_card.id}) at {player1_position}"
        )
        logger.info(
            f"Round {self.current_round}: Player 2 using {player2_card.name} (ID: {player2_card.id}) at {player2_position}"
        )
        ...
        if self.current_round % 2 == 1:
            ...
            result, details = MatchEngine.simulate_round(
                player1_card,
                player1_position,
                self.player1_formation,
                self.player1_team,
                player2_card,
                player2_position,
                self.player2_formation,
                self.player2_team,
            )
            ...
            logger.info(
                f"  P1 Card: {details['attacker']['card']}, Attack Roll: {details['attacker']['roll']}"
            )
            logger.info(
                f"  P2 Card: {details['defender']['card']}, Defense Roll: {details['defender']['roll']}"
            )
            ...
            if result == "attacker_wins":
                ...
                logger.info(
                    f"  Player 1 wins! Score: {self.player1_score} - {self.player2_score}"
                )
            elif result == "defender_wins":
                self.player2_score += 1
                winner_this_round = self.player2_id
                logger.info(
                    f"  Player 2 wins! Score: {self.player1_score} - {self.player2_score}"
                )
            else:
                winner_this_round = None
                logger.info(
                    f"  Draw! Score: {self.player1_score} - {self.player2_score}"
                )
        else:
            ...
            logger.info(
                f"  P2 Card: {details['attacker']['card']}, Attack Roll: {details['attacker']['roll']}"
            )
            logger.info(
                f"  P1 Card: {details['defender']['card']}, Defense Roll: {details['defender']['roll']}"
            )
        ...
        logger.info(
            f"Round {self.current_round} complete. Stored: P1={player1_card.name} (ID: {player1_card.id}), P2={player2_card.name} (ID: {player2_card.id})"
        )

        # Switch turn
        self.current_turn = (
            self.player2_id if self.current_turn == self.player1_id else self.player1_id
        )
        self.current_round += 1
    ...
    def is_complete(self) -> bool:
        """Check if match is complete (either max rounds reached or mercy rule triggered)"""
        # Check if max rounds reached
        if self.current_round > self.max_rounds:
            return True

        # Explicit mercy rule: 6 or more goal lead ends the match immediately
        score_difference = abs(self.player1_score - self.player2_score)
        if score_difference >= 6:
            return True

        # Insurmountable lead: If the losing team can't catch up even if they win all remaining rounds
        rounds_remaining = self.max_rounds - self.current_round + 1
        if score_difference > rounds_remaining:
            return True
    ...
    def is_mercy_rule_triggered(self) -> bool:
        """Check if mercy rule was triggered (6+ goal lead)"""
        return abs(self.player1_score - self.player2_score) >= 6
